...
class ExplainabilityRepository:
    ...
    def latest_assets(self, project_id: str):
        ...
        frame_path = cleaned.preprocessing_summary.get("engineered_frame_path") if cleaned.preprocessing_summary else None
        df = read_dataframe(Path(frame_path or cleaned.cleaned_parquet_path))
        return dataset, df, results
